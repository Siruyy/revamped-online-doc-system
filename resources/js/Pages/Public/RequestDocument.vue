...
function submit() {
    ...
    form.post(route('public.requests.store'), { forceFormData: true });
}
    ...
    <main class="min-h-screen bg-slate-50 text-slate-900">
        ...
        <form class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8" @submit.prevent="submit">
            <ol class="mb-8 grid gap-3 sm:grid-cols-4">
                <li
                    v-for="(label, index) in ['Documents', 'Requestor', 'Files & payment', 'Review']"
                    :key="label"
                    ...
                    "
                >
                    <span class="mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-white text-xs">
                        {{ index + 1 }}
                    </span>
                    {{ label }}
                </li>
            ...
            <section v-if="step === 1" class="space-y-6">
                ...
                >
                    <div class="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
                        ...
                        <h2 class="text-sm font-semibold uppercase tracking-wider text-slate-600">{{ category }}</h2>
                    </div>
                    <div class="divide-y divide-slate-100">
                        <article v-for="doc in docs" :key="doc.id" class="grid gap-4 px-5 py-4 md:grid-cols-[1fr_auto]">
                            ...
                            <div v-if="cart[doc.id]" class="flex items-center gap-2 md:justify-end">
                                <button
                                    type="button"
                                    class="rounded-full p-1 text-slate-500 hover:text-brand-700"
                                ...
                                <input
                                    ...
                                    min="1"
                                    max="20"
                                    class="w-16 rounded-xl border-slate-300 text-center text-sm"
            ...
            >
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea
                        ...
                        required
                    />
                    <p v-if="form.errors.purpose" class="mt-1 text-sm text-rose-600">{{ form.errors.purpose }}</p>
                </div>
            </section>

            <section v-else-if="step === 3" class="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
                <div class="space-y-4">
                    <div class="flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-slate-600">
                    ...
                    <FileUploadField
                        ...
                        :hint="requirement.hint || 'Upload a clear scanned copy or photo.'"
                ...
                <div class="space-y-4">
                    ...
                    <div class="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                        ...
                        <label class="mt-4 block text-sm font-semibold text-slate-700" for="payment_reference_number"
            ...
            <section v-else class="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
                <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                    ...
                    <ul class="mt-5 divide-y divide-slate-100 rounded-2xl border border-slate-200">
                        ...
                        >
                            <span>{{ item.type.name }} × {{ item.copies }}</span>
                ...
                <div class="rounded-3xl bg-slate-950 p-6 text-white shadow-xl">
                    ...
                    <button
                        type="submit"
                        class="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-accent-500 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-accent-600 disabled:opacity-60"
                        :disabled="form.processing"
                    >
                        Submit public request <CheckCircleIcon class="h-5 w-5" />
                    </button>
