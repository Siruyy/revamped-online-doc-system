    ...
    <StaffLayout>
        ...
        <div class="mx-auto max-w-7xl space-y-8 px-4 pb-12 sm:px-6 lg:px-8">
            ...
            <section class="grid gap-4 sm:grid-cols-3">
                <Link :href="route('department.clearances.index', { status: 'pending' })" class="group rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 p-6 text-white shadow-md transition hover:-translate-y-0.5">
                    ...
                    <p class="mt-3 inline-flex items-center gap-1 text-xs font-semibold opacity-90 group-hover:underline">Open queue <ArrowRightIcon class="h-3.5 w-3.5" /></p>
                </Link>

                <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                    <div class="flex items-center gap-3">
                        <div class="rounded-xl bg-emerald-100 p-2.5 text-emerald-700"><CheckCircleIcon class="h-5 w-5" /></div>
            ...
            <section class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                    ...
                    <Link :href="route('department.clearances.index')" class="text-xs font-semibold text-brand-700 hover:underline">
                        View all →
                    </Link>
                </div>
                <ul v-if="pendingLatest.length" class="divide-y divide-slate-100">
                    <li v-for="row in pendingLatest" :key="row.id" class="flex items-center justify-between px-6 py-3 transition hover:bg-brand-50/30">
                        ...
                        <div class="flex items-center gap-3">
                            <span class="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs text-slate-600">{{ relativeAge(row.created_at) }}</span>
                            <Link :href="route('department.clearances.show', row.id)" class="inline-flex items-center gap-1 rounded-lg bg-brand-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-brand-500">
                ...
                <div v-else class="px-6 py-12 text-center">
                    ...
                    <p class="text-xs text-slate-500">Your inbox is clear. New clearances will appear here in real time.</p>
                </div>
            </section>
        </div>
    </StaffLayout>
