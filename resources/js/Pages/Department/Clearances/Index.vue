...
const applyFilters = () => router.get(route('department.clearances.index'), form, { preserveState: true, replace: true });
const clearFilters = () => {
        ...
        <div class="mx-auto max-w-7xl space-y-5 px-4 pb-12 sm:px-6 lg:px-8">
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                ...
                <div class="grid gap-3 md:grid-cols-12">
                    ...
                    <input v-model="form.course" type="text" placeholder="Course" class="rounded-lg border-slate-300 text-sm shadow-sm md:col-span-3" />
                    <input v-model="form.year" type="number" min="1" max="8" placeholder="Year" class="rounded-lg border-slate-300 text-sm shadow-sm md:col-span-1" />
                    <input v-model="form.search" type="text" placeholder="Student name or ID" class="rounded-lg border-slate-300 text-sm shadow-sm md:col-span-4" @keyup.enter="applyFilters" />
                ...
                <table class="min-w-full divide-y divide-slate-100 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="row in clearances.data" :key="row.id" class="transition hover:bg-brand-50/30">
                            <td class="px-5 py-3 font-mono text-xs text-slate-700">{{ row.document_request?.reference_no }}</td>
                            <td class="px-5 py-3">
                                <p class="font-semibold text-slate-900">{{ row.user?.fullname }}</p>
                                <p class="text-xs text-slate-500">{{ row.user?.course }} · Y{{ row.user?.year_level }}</p>
                            </td>
                            <td class="px-5 py-3">
                                <span :class="['inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize', badge(row[departmentStatusColumn])]">
                                    {{ row[departmentStatusColumn] }}
                                </span>
                            </td>
                            <td class="px-5 py-3">
                                <span :class="['inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize', badge(row.overall_status)]">
                                    {{ row.overall_status?.replaceAll('_', ' ') }}
                                </span>
                            </td>
                            <td class="px-5 py-3 text-right">
                                <Link :href="route('department.clearances.show', row.id)" class="inline-flex items-center gap-1 rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-slate-700">
                                    Open <ArrowRightIcon class="h-3.5 w-3.5" />
                                </Link>
                            </td>
                        </tr>
                        <tr v-if="clearances.data.length === 0">
                            <td colspan="5" class="px-4 py-12 text-center text-sm text-slate-500">No clearances match these filters.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
