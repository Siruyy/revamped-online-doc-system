            ...
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <form class="mt-3 grid gap-3 md:grid-cols-2" @submit.prevent="createAnnouncement">
                    <input
                        ...
                        type="text"
                        placeholder="Title"
                        class="rounded-md border-slate-300 text-sm shadow-sm md:col-span-2"
                    />
                    <textarea
                    ...
                    <button
                        type="submit"
                        class="rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700 md:col-span-2"
                    >
                        Create
                    </button>
                </form>
            ...
            <section class="space-y-3">
                ...
                >
                    <form class="grid gap-3 md:grid-cols-2" @submit.prevent="updateAnnouncement(item.id)">
                        <input
                            v-model="forms[item.id].title"
                            type="text"
                            class="rounded-md border-slate-300 text-sm shadow-sm md:col-span-2"
                        />
                        <textarea
                            v-model="forms[item.id].body"
                            rows="3"
                            class="rounded-md border-slate-300 text-sm shadow-sm md:col-span-2"
                        />
                        ...
                        <label class="inline-flex items-center gap-2 text-sm text-slate-700 md:col-span-2">
                            <input
                                v-model="forms[item.id].is_published"
                                type="checkbox"
                                class="rounded border-slate-300 text-indigo-600 shadow-sm"
                            />
                            Published
                        ...
                        <div class="flex items-center justify-between md:col-span-2">
                            ...
                            <div class="flex gap-2">
                                ...
                                <button
                                    type="button"
                                    class="rounded-md bg-rose-600 px-3 py-2 text-xs font-semibold text-white hover:bg-rose-500"
