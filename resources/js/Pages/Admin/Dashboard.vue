    ...
    <StaffLayout>
        <template #header>
            <div>
                ...
                <p class="text-sm text-slate-500">Triage requests, monitor SLAs, and clear bottlenecks across the registrar pipeline.</p>
            </div>
        </template>
        ...
        <div class="mx-auto max-w-7xl space-y-8 px-4 pb-12 sm:px-6 lg:px-8">
            ...
            <section class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <Link :href="route('admin.requests.index', { status: 'pending' })" class="group rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs uppercase tracking-wider text-slate-500">Pending review</p>
                            <p class="mt-2 text-3xl font-display font-bold text-slate-900">{{ requestCounts.pending }}</p>
                        </div>
                ...
                <Link :href="route('admin.payments.index', { status: 'pending_approval' })" class="group rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs uppercase tracking-wider text-slate-500">Payment proofs</p>
                            <p class="mt-2 text-3xl font-display font-bold text-slate-900">{{ paymentCounts.pending_approval }}</p>
                        </div>
                ...
                <Link :href="route('admin.requests.index', { status: 'approved' })" class="group rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md">
                    <div class="flex items-start justify-between">
                        <div>
            ...
            <section class="grid gap-6 lg:grid-cols-3">
                <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 lg:col-span-2">
                    <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                        ...
                        <Link :href="route('admin.requests.index', { status: 'pending' })" class="text-xs font-semibold text-brand-700 hover:underline">
                            View all →
                        </Link>
                    </div>
                    <div v-if="pendingQueue.length === 0" class="px-6 py-10 text-center text-sm text-slate-500">No pending requests right now. </div>
                    <ul v-else class="divide-y divide-slate-100">
                        <li v-for="item in pendingQueue" :key="item.id" class="flex items-center justify-between gap-3 px-6 py-3 transition hover:bg-brand-50/40">
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center gap-2">
                                    <p class="truncate font-semibold text-slate-900">{{ item.document_type?.name }}</p>
                                    <span v-if="item.requirements?.some(r => r.status === 'missing')" class="rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-semibold uppercase text-rose-700">
                                        Missing reqs
                                    </span>
                                </div>
                            ...
                            <div class="flex items-center gap-3">
                                <span class="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold uppercase text-slate-600">
                                    <ClockIcon class="mr-1 inline-block h-3 w-3" />
                                    {{ relativeAge(item.created_at) }}
                                </span>
                                <Link :href="route('admin.requests.show', item.id)" class="inline-flex items-center gap-1 rounded-lg bg-brand-600 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-brand-500">
                ...
                <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                    ...
                    <div v-if="claimToday.length === 0" class="px-6 py-10 text-center text-sm text-slate-500">No pickups scheduled.</div>
                    <ul v-else class="divide-y divide-slate-100">
            ...
            <section class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                ...
                <div v-if="overdueRequests.length === 0" class="px-6 py-10 text-center text-sm text-slate-500">No overdue requests. </div>
                <table v-else class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="item in overdueRequests" :key="item.id" class="hover:bg-rose-50/40">
                            ...
                            <td class="px-6 py-3 text-rose-700 font-semibold">{{ new Date(item.expected_release_on).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) }}</td>
                            <td class="px-6 py-3 text-right">
                                <Link :href="route('admin.requests.show', item.id)" class="text-brand-700 font-semibold hover:underline">Open</Link>
                            </td>
                        </tr>
                    </tbody>
                </table>
